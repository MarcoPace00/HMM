"""
HMM regime-switching trading pipeline — HMM_4: trade the leveraged proxy.

Identical machinery to HMM_2 (volatility-targeted long/flat HMM regime
strategy), but with a clean separation between the *signal/sizing* asset and
the *traded* asset:

  * Signals, model fitting/selection, and the vol-targeting leverage overlay
    are all computed on the liquid, well-behaved BASE asset (QQQ / SPY).
  * The realized P&L — every VAL-Sharpe model-selection score, every stitched
    TEST segment, the IS grid-search score, and the reported OOS metrics — is
    earned on the LEVERAGED proxy (TQQQ / UPRO). Actual 3x-ETF returns are used
    (not 3x*base), so daily-rebalance volatility decay is captured honestly.
  * Each pair is benchmarked against its BASE buy-and-hold (QQQ / SPY).

Rationale: the base ETF gives a cleaner regime signal and a stable vol-target
denominator, while the leveraged ETF is what you actually hold when long.
Because leverage is sized off base vol (target_vol on the base), the realized
strategy vol runs ~3x the target — that is the intended, asked-for behaviour.

Pairs (base -> traded, benchmark):
  QQQ -> TQQQ   (bench QQQ B&H)
  SPY -> UPRO   (bench SPY B&H)

For each pair:
  1. Download daily simple returns for base and leveraged proxy; align dates.
  2. Split 60% in-sample / 40% OOS (on the base calendar).
  3. Grid-search (TRAIN_WINDOW, VAL_WINDOW, TEST_WINDOW, THRESHOLD) on the
     ...
     TEST segments — earned on the leveraged proxy — produced by a walk-forward
     that, at every step, picks the best of 3 HMMs on the VAL window (also
     scored on the proxy) and trades it on the TEST window.
  4. Re-run the walk-forward with the chosen combo on the OOS part.
  5. Plot the stitched equity curves + drawdowns; legend carries OOS stats.
...
Signal (long/flat, no look-ahead) — computed on the BASE asset:
  next_state = filtered_posterior(x_{1:t-1}) @ transmat
  score      = next_state @ per_state_expected_direction
  signal_t   = 1 if score > 0 else 0      # applied to leveraged return_t
"""
...
@dataclass(frozen=True)
class Pair:
    """A signal/sizing asset, the leveraged proxy actually traded, and the
    buy-and-hold benchmark to compare against."""
    base: str        # signals, model fitting/selection, vol-target sizing
    trade: str       # leveraged ETF actually held when long
    benchmark: str   # buy-and-hold reference (normally == base)
...
class Config:
    ...
    # vol of the BASE asset tracks target_vol. De-risk-only by default (cap
    # leverage at 1.0). NB: sizing is on the base, but the position is held in
    # the leveraged proxy, so realized strategy vol runs well above target_vol.
    target_vol: float = 0.15               # annualized vol target (on base)
    vol_lookback: int = 20                 # trailing window for realized vol
    max_leverage: float = 1.0              # cap; 1.0 = never lever beyond fully long

    @property
    def tickers(self) -> tuple[str, ...]:
        """Unique download universe across all pairs (base + trade + bench)."""
        seen: dict[str, None] = {}
        for p in self.pairs:
            for tk in (p.base, p.trade, p.benchmark):
                seen.setdefault(tk, None)
        return tuple(seen)
...
def vol_target_leverage(ret: np.ndarray, cfg: Config, ann: int) -> np.ndarray:
    ...
    measured on the BASE asset returns `ret`. leverage_t uses only returns
    strictly before t (trailing realized vol shifted by one day), so it adds no
    look-ahead. Capped at cfg.max_leverage; 0 until vol_lookback history exists
    (i.e. flat during warm-up).
    """
    s = pd.Series(ret)
...
# Walk-forward                                                                #
# --------------------------------------------------------------------------- #
def walk_forward(ret: np.ndarray, trade_ret: np.ndarray, dates: pd.DatetimeIndex,
                 hp: HParams, cfg: Config, ann: int, ts_start: int, ts_end: int,
                 ) -> tuple[pd.Series, Counter]:
    """
    Tile non-overlapping TEST segments over [ts_start, ts_end).
    Each step: fit all models on the BASE TRAIN window, pick the spec with the
    best VAL-Sharpe (scored on the LEVERAGED proxy), trade it on TEST (P&L also
    on the leveraged proxy). Signals and vol-target leverage are derived purely
    from the base series `ret`; only the realized return uses `trade_ret`.
    `ret` and `trade_ret` are aligned 1:1 on the same `dates` calendar.
    ...
    lev = vol_target_leverage(ret, cfg, ann)   # causal sizing off the base asset
    ...
    while ts + hp.test_window <= ts_end:
        ...
        for spec in MODELS:
            ...
            # model selection scored on what we actually trade: the leveraged proxy
            val_sh = safe_sharpe(val_sig * lev[val_lo:ts] * trade_ret[val_lo:ts], ann)
            if val_sh > best_val:
        ...
        if best_model is not None:
            ...
            # realized P&L on the leveraged proxy, sized off the base asset
            test_ret = (test_sig * lev[ts:ts + hp.test_window]
                        * trade_ret[ts:ts + hp.test_window])
...
@dataclass
class AssetResult:
    label: str                 # legend label, e.g. "TQQQ (sig QQQ)"
    base: str
    trade: str
    best_hp: HParams
...
def run_pair(pair: Pair, base_s: pd.Series, trade_s: pd.Series,
             cfg: Config) -> AssetResult | None:
    # Align signal/sizing (base) and traded (leveraged) series to a shared
    # calendar so each base signal maps to the proxy's realized return that day.
    common = base_s.index.intersection(trade_s.index)
    base_s = base_s.loc[common]
    trade_s = trade_s.loc[common]
    if len(common) < 600:
        log.warning("%s->%s: only %d aligned days", pair.base, pair.trade, len(common))

    ret = base_s.to_numpy(float)
    trade_ret = trade_s.to_numpy(float)
    dates = base_s.index
    n = len(ret)
    split = int(n * cfg.split_frac)
    ann = ann_factor(pair.base)
    label = f"{pair.trade} (sig {pair.base})"
    ...
    for hp in cfg.grid():
        ...
        is_ret, is_picks = walk_forward(ret, trade_ret, dates, hp, cfg, ann,
                                        is_start, split)
    ...
    if best_hp is None:
        log.warning("%s: no valid hyper-parameter combo", label)
        return None
    log.info("%-16s best IS sharpe=%.2f  %s", label, best_score, best_hp)
    log.info("%-16s IS  model picks: %s", label, fmt_picks(best_is_picks))

    # ---- OOS walk-forward with chosen combo --------------------------------
    oos_ret, oos_picks = walk_forward(ret, trade_ret, dates, best_hp, cfg, ann,
                                      split, n)
    if oos_ret.empty:
        log.warning("%s: empty OOS", label)
        return None
    log.info("%-16s OOS model picks: %s", label, fmt_picks(oos_picks))
...
def plot_results(results: list[AssetResult], out_html: str,
                 benchmarks: dict[str, pd.Series] | None = None) -> go.Figure:
    palette = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd",
               "#8c564b", "#e377c2"]
    bench_colors = ["#555555", "#999999", "#bbbbbb"]
    fig = make_subplots(
    ...
    # Align every curve to a common start so the comparison is fair. Pairs have
    # different tuned warm-ups, so clip each curve to the latest per-pair start
    # and renormalise to 1 there; drawdown is recomputed over that visible
    ...
    # Buy-and-hold benchmarks (QQQ / SPY) over the same displayed window —
    # always invested, no tuning — normalised to 1 at common_start.
    for j, (name, series) in enumerate((benchmarks or {}).items()):
        bench = series.loc[common_start:]
        if bench.empty:
            continue
        ...
        color = bench_colors[j % len(bench_colors)]
        lbl = (f"{name} B&H | Sh {safe_sharpe(bench.to_numpy(), 252):.2f} | "
               f"CAGR {cagr(bench_np, 252):.1%} | DD {max_drawdown(bench_np):.1%}")
        fig.add_trace(
            go.Scatter(x=bench_eq.index, y=bench_eq.values, name=lbl,
                       legendgroup=f"bench_{name}",
                       line=dict(color=color, width=2.0, dash="dash")),
            row=1, col=1)
        fig.add_trace(
            go.Scatter(x=bench_dd.index, y=bench_dd.values, name=lbl,
                       legendgroup=f"bench_{name}", showlegend=False,
    ...
    for i, r in enumerate(results):
        ...
        eq = eq / eq.iloc[0]
        dd = eq / eq.cummax() - 1.0
        label = (f"{r.label} | Sh {r.oos_sharpe:.2f} | "
                 f"CAGR {r.oos_cagr:.1%} | DD {r.oos_maxdd:.1%}")
        fig.add_trace(
            go.Scatter(x=eq.index, y=eq.values, name=label,
                       legendgroup=r.label, line=dict(color=c, width=1.6)),
            row=1, col=1)
        fig.add_trace(
    ...
    # 60% split reference (the equity ETFs share a calendar)
    ref = results[0]
    for row in (1, 2):
    ...
    fig.update_layout(
        title=f"HMM regime strategy — signal/size on base, trade leveraged proxy "
              f"(OOS metrics in legend, split @ {ref.split_date.date()})",
...
def main(cfg: Config | None = None,
         ...
         out_html: str = "hmm_equity_leveraged.html",
         n_jobs: int | None = None) -> go.Figure:
    """
    Run every pair through `run_pair` and plot the results.

    Pairs are independent and CPU-bound, so each is processed in its own
    process. `n_jobs` caps the worker count (default: min(#pairs, #cpus));
    pass n_jobs=1 to force the serial path for debugging.
    """
    cfg = cfg or Config()
    returns = returns or download_returns(cfg)
    pairs = [p for p in cfg.pairs if p.base in returns and p.trade in returns]
    ...
    if n_jobs == 1 or len(pairs) <= 1:
        for p in pairs:
            res = run_pair(p, returns[p.base], returns[p.trade], cfg)
            if res is not None:
                results.append(res)
    else:
        max_workers = min(len(pairs), n_jobs or os.cpu_count() or 1)
        log.info("running %d pairs across %d processes", len(pairs), max_workers)
        with ProcessPoolExecutor(max_workers=max_workers) as ex:
            futs = {ex.submit(run_pair, p, returns[p.base], returns[p.trade], cfg): p
                    for p in pairs}
            for fut in as_completed(futs):
                ...
                try:
                    ...
                    log.exception("%s->%s: worker failed", p.base, p.trade)
                    continue
                if res is not None:
                    results.append(res)
        # restore the configured pair order (palette/legend stability)
        order = [p.trade for p in cfg.pairs]
        results.sort(key=lambda r: order.index(r.trade))

    if not results:
        raise RuntimeError("no pair produced results")

    # one benchmark line per distinct benchmark ticker, in pair order
    benchmarks: dict[str, pd.Series] = {}
    for p in cfg.pairs:
        if p.benchmark in returns:
            benchmarks.setdefault(p.benchmark, returns[p.benchmark])
    return plot_results(results, out_html, benchmarks=benchmarks)
