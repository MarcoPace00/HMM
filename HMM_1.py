"""
HMM regime-switching trading pipeline.
...
def walk_forward(ret: np.ndarray, dates: pd.DatetimeIndex, hp: HParams,
    ...
    while ts + hp.test_window <= ts_end:
        ...
        for spec in MODELS:
            ...
            val_sh = safe_sharpe(val_sig * ret[val_lo:ts], ann)
            if val_sh > best_val:
        ...
        if best_model is not None:
            ...
            test_ret = test_sig * ret[ts:ts + hp.test_window]
